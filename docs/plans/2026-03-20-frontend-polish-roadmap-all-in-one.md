...
- Additionally, the preview currently renders editor HTML via `dangerouslySetInnerHTML` without a visible allowlist/sanitization policy on the frontend, which creates potential XSS risk if stored content is not guaranteed safe.
...
- Implement sanitization/allowlist policy for `EditorPreview` rendering:
...
- Keyboard-only navigation walkthrough for each block.
- Screen reader spot-check for key flows: editor toolbar, preview, public navigation, filters pagination, 2FA, settings.
- Lighthouse/axe checks focusing on: nested interactive elements, landmark/name/role, labels/aria-invalid/aria-describedby, chart announcements.
...
3. [P0] Accessibility: improve `InputError` semantics for auth/editor forms (role + live region + optional id support). | Files: `resources/js/components/input-error.tsx` | Acceptance: errors are announced automatically (screen reader spot-check on an error state).
...
6. [P2] Security polish: confirm/safeguard sanitization of stored post HTML before rendering on public “Post”. | Files: `resources/js/pages/public/post.tsx` (and verify backend sanitization contract) | Acceptance: stored XSS payloads (if any) are neutralized; front-end rendering is safe.
